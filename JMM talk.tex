...
\title[Leibniz-type rules]{Leibniz-type rules and applications to scattering properties of PDEs II}
\author[Alex Thomson]{Alex Thomson}



\institute[Kansas State University]{
Department of Mathematics%\\[1ex]
% \begin{center}
...
\begin{frame}\frametitle{Kato--Ponce inequalities}
...
For $1<p<\infty$ and $s\ge 0,$ 
\begin{align*}
\norm{J^s(fg)}{L^p} &\lesssim   \norm{J^sf}{L^p} \norm{g}{L^\infty}+ \norm{f}{L^{\infty}} \norm{J^sg}{L^p},\\
& \\
\norm{J^s(fg)-fJ^s(g)}{L^p} &\lesssim  \norm{J^sf}{L^p} \norm{g}{L^\infty}+ \norm{\nabla f}{L^\infty} \norm{J^{s-1}g}{L^p}.
\end{align*}
 
 \bigskip

Such estimates played an important role in the treatment by Kato and Ponce (1988) of the Cauchy problem for the Euler and Navier--Stokes equations in the setting of Sobolev spaces. 
\end{frame}
...
where 
\begin{align*}
&1<p_1,p_2\le \infty, \fr{1}{2}<p\le \infty, \hcline, \\
&s\in 2\na_0 \text{ or } s>\tau_p,  \tau_p:=n({1}/{\min(p,1)}-1).
\end{align*}
(Different choices of $p_1$ and $p_2$ can be used on the right-hand sides of the inequalities above.)
...
\begin{theorem}[N.--Thomson, 2018]\label{thm:CM:TL:B}  
...
For $s \ge 0$ set $D^s:=(-\Delta)^{s/2},$ that is,
\begin{align*}
...
\begin{frame}{Definitions: Muckenhoupt Weights}
\begin{itemize}
...
\begin{frame}{Definitions: Weighted Triebel-Lizorkin Spaces}
...
\end{itemize}
\bigskip
For $s\in\re,$  $0<p<\infty,$  $0<q\le \infty$ and $w\in A_\infty,$   the homogeneous weighted Triebel-Lizorkin space $\tlw{p}{s}{q}{w}$ consists of all $f\in \swp/\mathcal{P}(\rn)$ such that 
\begin{equation*}
...
\begin{frame}{Definitions: Weighted Hardy spaces}
...
\begin{corollary}[N.--Thomson, 2018]\label{coro:KP:CM:Hardy}  Let $\sigma(\xi,\eta),$ $\xi,\eta\in\rn,$ be a Coifman-Meyer multiplier. 
