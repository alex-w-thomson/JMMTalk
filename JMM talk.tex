...
\item Boundedness properties of operators associated to Coifman-Meyer multipliers are well known. In the setting of weighted Lebesgue spaces \[ ||T_\sigma(f,g)||_{L^p(w)} \lesssim ||f||_{L^{p_1}(w)} ||g||_{L^{p_2}(w)} \] where $\sigma$ is a Coifman-Meyer multiplier, $\hcline$, and $1<p_1,p_2<\infty$.
\end{itemize}
...
\end{itemize}
\bigskip
For $s\in\re,$  $0<p<\infty,$  $0<q\le \infty$ and $w\in A_\infty,$ the space $\tlw{p}{s}{q}{w}$ consists of all $f\in \swp/\mathcal{P}(\rn)$ such that 
\begin{equation*}
